...
/// This ensures that all API controllers have access to the <see cref="UnitOfWork" />, basic CRUD operation and other helpful controller methods like <see cref="BaseController.InternalServerError"/>.
...
public abstract class BaseController : ControllerBase
{
        ...
        where TRepository : BaseRepository<TEntity, Guid> // The repository must be a BaseRepository with a Guid as Id
    {
        try
        {
            ...
            bool exists = repository.Exists(payload.Id.Value);
            if (!exists) return NotFound(payload.Id.Value);

            // Update the entity in the database and save changes
            repository.Update(payload.Adapt<TEntity>());
            await uow.SaveChangesAsync();
